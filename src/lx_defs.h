...
/* compteur 25MHz entre 8 ech. */
#define XES_FREQ_COUNT8_MASK    0x00001FFF
/* 25M / [ 44k - ( 44.1k + 48k ) / 2 ] * 8 */
#define XES_FREQ_COUNT8_44_MIN  0x00001288
/* 25M / [ ( 44.1k + 48k ) / 2 ] * 8 */
#define XES_FREQ_COUNT8_44_MAX  0x000010F0
/* 25M / [ 48k + ( 44.1k + 48k ) / 2 ] * 8 */
#define XES_FREQ_COUNT8_48_MAX  0x00000F08

/* code adapted from LXES_registers.h */
/* (rw) offset for the number of OUTs in the ConfES register. */
#define IOCR_OUTPUTS_OFFSET 0
/* (rw) offset for the number of INs in the ConfES register. */
#define IOCR_INPUTS_OFFSET  8
/* (rw) offset for frequency ratio in the ConfES register. */
#define FREQ_RATIO_OFFSET  19
/* value for single mode frequency ratio: sample rate = frequency rate. */
...
/* events that lead to a PCI irq if not yet pending */
...
/* reserved, this is set by the XES */
...
/* event that remains internal: reserved fo end of plx dma */
#define MASK_SYS_STATUS_EOT_PLX  (1L << 22)
/* event that remains internal: pending XES IRQ */
#define MASK_SYS_STATUS_XES      (1L << 21)
/* alternate command management: notify driver instead of polling */
#define MASK_SYS_STATUS_CMD_DONE (1L << 20)

#define MAX_STREAM_BUFFER 5        /* max amount of stream buffers. */

#define MICROBLAZE_IBL_MIN                 8
#define MICROBLAZE_IBL_DEFAULT             64
#define MICROBLAZE_IBL_MAX                 512
#define MICROBLAZE_LX_PCI_PERIODS_MAX      5
#define MICROBLAZE_CHANNELS_MAX            64
...
/* object ID is at the 13th bit in the 1st command word.*/
#define ID_OFFSET     13
#define ID_CH_MASK    0x3F
/* offset of the command opcode in the first command word.*/
...
enum cmd_mb_opcodes {
	...
	/* specific madi */
	CMD_14_GET_MADI_STATE = 0x14,
	CMD_15_SET_MADI_STATE = 0x15,
...
/* pipe states */
enum pipe_state_t {
	/* the pipe is not processed in the XES_IRQ
	 * (free or stopped, or paused).
	 */
	PSTATE_IDLE = 0,
	/* sustained play/record state. */
	PSTATE_RUN = 1,
	/* the ES channels are now off, render pipes do
	 * not DMA, record pipe do a last DMA.
	 */
	PSTATE_PURGE = 2,
	/* the ES channels are now on, render pipes do
	 * not yet increase their sample count, record
	 * pipes do not DMA.
	 */
	PSTATE_ACQUIRE = 3,
	/* the pipe is releasing, and may not yet
...
/* stream states */
enum stream_state_t {
	/* setting to stop resets the stream spl count.*/
	SSTATE_STOP = 0x00,
	/* start DMA and spl count handling. */
	SSTATE_RUN = (0x01 << 0),
	/* pause DMA and spl count handling. */
	SSTATE_PAUSE = (0x01 << 1),
};

/* buffer flags */
enum buffer_flags {
	/* set if the buffer is valid, clear if free.*/
	BF_VALID = 0x80,
	/* set if this is the current buffer
	 * (there is always a current buffer).
	 */
	BF_CURRENT = 0x40,
	/* set if this buffer must cause a PCI event when finished.*/
	BF_NOTIFY_EOB = 0x20,
	/* set if buffer[1] must be copied to buffer[0]
	 * by the end of this buffer.
	 */
	BF_CIRCULAR = 0x10,
	/* set if the hi part of the address is valid.*/
	BF_64BITS_ADR = 0x08,
	/* future extension.*/
	BF_xx = 0x04,
	/* set if finished, but not yet free.*/
	BF_EOB = 0x02,
	/* pause stream at buffer end.*/
	BF_PAUSE = 0x01,
	/* no flags (init).*/
	BF_ZERO = 0x00,
};
 ...
 */
enum stream_flags {
	/* no flags (stream invalid). */
	SF_ZERO = 0x00000000,
	/* the stream has a valid DMA_conf info (setstreamformat). */
	SF_VALID = 0x10000000,
	/* the stream is un x-run state. */
	SF_XRUN = 0x20000000,
	/* the DMA is running.*/
	SF_START = 0x40000000,
	/* ASIO.*/
...
#define MASK_STREAM_IS_ALSA            (1L <<  8)
/* the stream fmt bits start at the 10th bit in the command word. */
#define STREAM_FMT_OFFSET 10

#define STREAM_FMT_16b     0x02
#define STREAM_FMT_intel   0x01
/* offset of the freq field in the response word */
#define FREQ_FIELD_OFFSET  15
/*  offset of the buffer flags in the response word. */
#define BUFF_FLAGS_OFFSET  24
/* this must match the field size of datasize in the buffer_t structure. */
#define MASK_DATA_SIZE     0x00FFFFFF
/* the cancel command awaits a buffer ID, may be 0xFF for "current". */
